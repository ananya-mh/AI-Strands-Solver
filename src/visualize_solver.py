...
def draw_grid_with_words(grid, found_words, top_words, save_path=None):
    ...
    # Remove duplicate words based on string
    seen_words = set()
    unique_words = []
    ...
    # Highlight found words and connect paths
    ...
    # Side panel for top words
    fig.subplots_adjust(right=0.65)  # pull main plot left a bit more

    side_ax = fig.add_axes([0.67, 0.1, 0.3, 0.8])  # [left, bottom, width, height]
